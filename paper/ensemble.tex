...
SWA \citep{izmailov2018averaging}, SWAP \citep{gupta2020stochastic}, EMA
\citep{ho2020denoising}, and Model Soups \citep{wortsman2022model} all average model
weights to reach flatter minima or improve generalization, but do so sequentially or
after training.
HDET instead repeatedly averages $N$ simultaneously diverging replicas \emph{during}
training---generating richer diversity at the same wall-clock time.
...
NES \citep{wierstra2014natural}, ES \citep{salimans2017evolution}, Bayesian
optimization \citep{snoek2012practical}, and random search \citep{bergstra2011algorithms}
optimize hyperparameters across separate runs or via population fitness signals.
HDET's auto-LR uses inter-replica loss differences as a lightweight within-run signal,
requiring no restarts or separate evaluation infrastructure.
